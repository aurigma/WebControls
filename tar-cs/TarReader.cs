...
namespace tar_cs
{
    ...
    public class TarReader
    {
	    ...
	    public int Read(byte[] buffer, int offset, int count)
	    {
			...
			var bytesRemainingToRead = count < _remainingBytesInFile ? count : (int)_remainingBytesInFile;
		    var bytesRead = 0;
		    var totalBytesRead = 0;
			do
			{
				bytesRead = inStream.Read(buffer, offset, bytesRemainingToRead);

				totalBytesRead += bytesRead;
				bytesRemainingToRead -= bytesRead;
				_remainingBytesInFile -= bytesRead;
			} 
			while (bytesRead > 0 && bytesRemainingToRead > 0);
        ...
        public bool MoveNext(bool skipData)
        {
            ...
            if (_remainingBytesInFile > 0)
            {
                ...
                if (inStream.CanSeek)
                {
                    ...
                    inStream.Seek(_remainingBytesInFile + (512 - (remainer == 0 ? 512 : remainer) ), SeekOrigin.Current);
                }
                else
                {
                    byte[] buffer;
            ...
            _remainingBytesInFile = header.SizeInBytes;
